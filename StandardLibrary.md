...
> `sys` 모듈은 [프로그램의 입출력(sys.argv)](ProgramIO.md)에서, `re` 모듈은 정규 표현식에서 따로 다룬다.
...
- **패턴 문법**
  ...
  | `**` | 하위 디렉터리 전체, `recursive=True`와 함께 써야 모든 깊이를 의미한다 | `Source/**/*.py` |
...
- **`glob(pathname)`** — 패턴에 맞는 파일 경로를 리스트로 반환한다.
  ...
  print(paths)
  # ['Source/datatype/tuple.py', 'Source/datatype/list.py', 'Source/datatype/bool.py', ...]
  
  # Source 아래 모든 깊이의 .py 반환
  paths = glob.glob("Source/**/*.py", recursive=True)
  print(paths)
  # ['Source/hello_vscode.py', 'Source/advanced/built_in_function.py', ..., 'Source/packages/main/package.py']
  ```
...
#   File ".../Source/advanced/standard_library.py", line 448, in main
#     b()
#     ~^^
#   File ".../Source/advanced/standard_library.py", line 445, in b
#     a()
#     ~^^
#   File ".../Source/advanced/standard_library.py", line 443, in a
#     return 1/0
#            ~^~
