...
> 반복 가능한 데이터란 `for` 문에서 사용할 수 있는 자료형을 말한다. 리스트, 튜플, 문자열, 딕셔너리, 집합 등이 있다. 자료형의 참과 거짓은 [불(Bool)](../Chapter2/Bool.md)을 참고한다.
...
> `[]`는 괄호 안의 내용을 생략할 수 있다는 관례 표기법이다. 자세한 사용법은 [사용자 입출력(input/print)](../Chapter4/UserIO.md)을 참고한다.
...
> `b`는 `w`, `r`, `a`와 함께 사용한다. 파일 다루기는 [파일 읽고 쓰기(file I/O)](../Chapter4/FileIO.md)에서 자세히 다룬다.
