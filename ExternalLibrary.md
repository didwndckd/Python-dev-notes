...
> 외부 라이브러리는 프로젝트마다 필요한 버전이 다르므로 [가상 환경(venv)](VirtualEnvironment.md)을 활성화한 상태에서 설치하는 것이 좋다. 아래 예제도 가상 환경이 활성화되어 있다고 가정한다.
...
- **테스트 데이터 만들기** — [리스트 컴프리헨션](For.md)과 함께 쓰면 원하는 건수만큼 한 번에 만들 수 있다.
